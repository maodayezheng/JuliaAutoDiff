#function DemoMNISTclass()
    ...
    Ntrain=2000
    BatchSize=1000
    TrainingIts=250 # number of Nesterov updates
    ...
    data=images
    class=BitArray(10,70000); fill!(class,false)
    for i=1:70000
        class[label[i]+1,i]=true
    end
    ...
    node[loss=c+=1]=ADnode([classtrain h[L]],FMultLogisticLoss) # the loss we are minimising must be the final node in the graph.
